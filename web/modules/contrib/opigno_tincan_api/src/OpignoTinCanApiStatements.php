...
/**
 * Provides a service to work with TinCan statements API.
 *
 * @package Drupal\opigno_tincan_api
 */
class OpignoTinCanApiStatements {
  ...
  public static function sendStatement(Statement $statement) {
    // Don't create a queue if TinCan LRS data is empty.
    ...
    $pwd = $config->get('opigno_tincan_api_password') ?? '';
    if (!$endpoint || !$username || !$pwd) {
      return FALSE;
    }
