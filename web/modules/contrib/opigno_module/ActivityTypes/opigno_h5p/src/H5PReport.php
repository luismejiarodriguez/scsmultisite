...
class H5PReport {
  ...
  public function generateReport(object $xapiData, ?string $forcedProcessor = NULL, bool $disableScoring = FALSE): string {
    ...
    $interactionType = $this->determineInteractionType($xapiData, $forcedProcessor);

    if (!isset($this->processors[$interactionType])) {
      $this->initializeProcessor($interactionType);
    }

    if (!isset($this->processors[$interactionType])) {
      // No processor found or initialized.
      return '';
    }

    // Generate and return report from xAPI data.
    return $this->processors[$interactionType]->generateReport($xapiData, $disableScoring, ($interactionType == "compound"));
  }
   ...
   */
  protected function determineInteractionType(object $xapiData, ?string $forcedProcessor): string {
  ...
  /**
   * Initialize the processor for the given interaction type.
   *
   * @param string $interactionType
   *   The interaction type.
   */
  protected function initializeProcessor(string $interactionType): void {
    if (array_key_exists($interactionType, self::$contentTypeProcessors)) {
      $processorClass = self::$contentTypeProcessors[$interactionType];
    }
    elseif (array_key_exists($interactionType, self::$processorMap)) {
      $processorClass = self::$processorMap[$interactionType];
    }
    else {
      return;
    }

    $this->processors[$interactionType] = new $processorClass();
  }
   ...
   *   A report.
   *
   * @todo Seems this method is not used anywhere starting from it was written.
   */
  public function generateGradableReports(object $xapiData): string {
    ...
    foreach ($xapiData as $childData) {
      ...
      if (!isset($this->processors[$interactionType])
        ...
        $processorClassName = self::$contentTypeProcessors[$interactionType];
        $this->processors[$interactionType] = new $processorClassName();
      }
    ...
    return count($results) > 0 ? $this->buildContainer($results) : '';
  }
