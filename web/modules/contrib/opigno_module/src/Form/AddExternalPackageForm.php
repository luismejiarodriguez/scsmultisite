...
class AddExternalPackageForm extends FormBase {
  ...
  public function validateForm(array &$form, FormStateInterface $form_state) {
    ...
    if (!$file[0]) {
      // Set a custom form error if the file cannot be saved. The function
      // file_save_upload only generates error messages without attaching them
      // to the form, preventing extraction of the original error message.
      $form_state->setError(
        $form[$file_field],
        $this->t('The file cannot be saved. Please check the file size limit and allowed file extensions.')
      );
