...
class OpignoModule extends RevisionableContentEntityBase implements OpignoModuleInterface {
  ...
  public function getHideResults($answer = NULL): bool {
    ...
    $result = (bool) $this->get('hide_results')->value;
    ...
    if ($user_module_status instanceof UserModuleStatus &&
      $user_module_status->hasField('learning_path') &&
      !empty($group = $user_module_status->get('learning_path')->target_id) &&
      LearningPathAccess::memberHasRole('user_manager', $account, $group)) {
      return FALSE;
    }
  ...
  public function getModuleAttempts(
    ...
    if (isset($group_id)) {
      $cid .= '_' . $group_id;
      if ($last_lp_attempt) {
        $cid .= '_' . $last_lp_attempt;
      }

      $key = $cid . ($range ? '_' . $range : '');
      ...
      if ($status_ids) {
        ...
        $this->userAttempts[$cid] = $status_entities;
        ...
        $this->userAttempts[$cid . '_last'] = [$max_id => $status_entities[$max_id]];
        $max_score = 0;
        $best_entity = FALSE;
        ...
        if (!$best_entity) {
          $this->userAttempts[$cid . '_best'] = [];
        }
        else {
          $this->userAttempts[$cid . '_best'] = [$best_entity->id() => $best_entity];
        }
      }
      else {
        $this->userAttempts[$cid . '_best'] = [];
        $this->userAttempts[$cid . '_last'] = [];
      }
  ...
  public function getUserScore(AccountInterface $account, $latest_cert_date = NULL, $group_id = NULL) {
    $group_id = $this->getGroupIdCurrentTraining($group_id);
    $attempts = $this->getModuleAttempts($account, 'best', $latest_cert_date, FALSE, $group_id);
    if (!$attempts) {
      return 0;
    }
