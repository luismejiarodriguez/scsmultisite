...
class OpignoUserSelectionController extends ControllerBase {
   ...
   * @opigno_deprecated
   */
  public function getUserAvatar($user, &$meta) {
    $context = new RenderContext();

    $image_style_url = \Drupal::service('renderer')->executeInRenderContext($context, function () use ($user) {
    ...
    if (!$context->isEmpty()) {
      ...
      $meta = $meta->merge($bubbleable_metadata);
    }
  ...
  public function post($data = NULL) {
    ...
    $response_data['users'] = (array_map(function ($user) use (&$groups_id, $meta, $map) {
      ...
      return [
        ...
        'avatar' => $this->getUserAvatar($user, $meta),
