...
use Drupal\opigno_moxtra\MoxtraService;
...
function opigno_moxtra_post_update_update_user_status(&$sandbox) {
  ...
  if (!empty($org_id) && $status === TRUE) {
    ...
    if (!isset($sandbox['last_uid'])) {
      $sandbox['last_uid'] = 0;
      $sandbox['current'] = 0;
      $sandbox['total'] = \Drupal::entityQuery('user')->accessCheck()->count()->execute();
    }
    ...
    foreach ($users as $user) {
      $active = $user->isActive() && $user->hasRole(MoxtraService::OPIGNO_MOXTRA_COLLABORATIVE_FEATURES_RID);
      if ($active) {
        $data = [
          'uid' => $user->id(),
