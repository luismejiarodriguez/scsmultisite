...
class OpignoLikeManager {
   ...
   *   The way "Likers" link should look like: "icon" or "text".
   * @param string $type
   *   The entity type.
   *
   * @return array
   ...
   */
  public function getEntityLikeLink(int $eid, string $link_type = 'icon', string $type = 'opigno_post'): array {
   ...
   */
  public function getLikersCountLink(int $eid, string $link_type = 'icon', string $type = 'opigno_post'): array {
    $likers = $this->getEntityLikers($eid, $type);
    $count = count($likers);
    // Prepare the link to get likers.
    if ($link_type === 'text') {
      $title = $this->formatPlural($count, '1 Like', '@count Likes');
    }
    else {
