...
class TrainingsCatalogueRestResource extends ResourceBase {
  ...
  public function get() {
    ...
    $response_data = \Drupal::service('renderer')->executeInRenderContext(new RenderContext(), function () {
      ...
      $title_sort = $request_query->get('title_sort') &&
      in_array(strtoupper($request_query->get('title_sort')), ['ASC', 'DESC'])
        ? $request_query->get('title_sort') : NULL;
      // Get sort parameter by date created.
      $created_sort = $request_query->get('created_sort') &&
      in_array(strtoupper($request_query->get('created_sort')), ['ASC', 'DESC'])
