...
/**
 * Provides a resources and callbacks for the LearningPath.
 */
class LearningPathController extends ControllerBase {
  ...
  public function getLatestActiveTrainings() {
    ...
    $limit = $this->request->query->get('limit') ?: $default_limit;
    // Get trainings ids.
    ...
    foreach ($latest_trainings as $training) {
      ...
      if ($tid && $term = $this->entityTypeManager()
        ->getStorage('taxonomy_term')
        ->load($tid)) {
        $training_category = [
          'id' => $term->id(),
  ...
  public function getGroupsInfoByUserMembership() {
    ...
    $group_type = $this->request->query->get('group_type');
