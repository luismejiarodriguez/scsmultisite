...
trait GroupRouteContextTrait {
  ...
  public function getGroupFromRoute() {
    ...
    elseif ($route_match->getRouteName() == 'views.ajax') {
      switch (\Drupal::request()->getMethod()) {
        case 'GET':
          $view_path = $_GET['view_path'] ?? NULL;
          break;
        case 'POST':
          $view_path = $_POST['view_path'] ?? NULL;
          break;
      }
      if ($view_path && $view_route_match = \Drupal::service('router.no_access_checks')->match($view_path)) {
        // See if the route has a group parameter and try to retrieve it.
        if (!empty($group = $view_route_match['group']) && $group instanceof GroupInterface) {
          return $group;
        }
      }
    }
