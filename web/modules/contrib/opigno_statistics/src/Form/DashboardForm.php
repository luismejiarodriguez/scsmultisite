...
class DashboardForm extends FormBase {
   ...
   *   Render array.
   */
  protected function buildUsersPerDay(array $period, array $lp_ids = []): array {
    $query = $this->database
    ...
    $data = $query
      ->condition('u.date', $period, 'BETWEEN')
      ->groupBy('hour')
      ->execute()
      ->fetchAllAssoc('hour');

    // Get the number of days in the month.
    $days = date('t', strtotime($period['min']));
   ...
   * @throws \Exception
   */
  protected function buildTrainingsProgress(array $period, array $lp_ids = []): array {
    $data = $this->getTrainingStistics($period, $lp_ids);
    return [
      'drupalSettings' => [
        'opignoCharts' => [
          'trainingsProgress' => $this->buildDonutChart(
            (float) $data['progress'], '#trainings-progress-chart'
          ),
          'trainingsCompletion' => $this->buildDonutChart(
            (float) $data['completion'], '#trainings-completion-chart'
          ),
          'usersPerDay' => $this->buildUsersPerDay($period, $lp_ids),
        ],
      ],
    ];
  }
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    $form['trainings_progress'] = [
      ...
      '#attached' => $this->buildTrainingsProgress($period, $lp_ids),
      'year' => $year_select,
    ];
