...
class UserController extends ControllerBase {
   ...
   */
  protected UserStatisticsManager $statsManager;
   ...
   */
  protected UserConnectionManager $connectionsManager;
   ...
   *   Opigno user statistics manager.
  ...
  public function buildUserInfo(UserInterface $user, bool $is_private, bool $can_access_private, bool $is_in_network) {
    $uid = (int) $user->id();
    $viewer_uid = $this->currentUser()->id();
    $same_user = $uid == $viewer_uid;
    $access_mode = $this->currentUserAccessMode($user);
    $build = [
    ...
    // Connection link.
    if (!$same_user && !$is_private && !$is_in_network) {
      $link_build = $this->connectionsManager->getUserConnectionLinks($uid, TRUE);
      $send_link = $link_build['#actions']['send'] ?? NULL;
      if ($send_link) {
        $send_link->setText($this->t('Connect with me'));
      }
      $build['data']['social_links']['connect_link'] = $link_build;
    }
    ...
    if (!$same_user && (!$is_private || $is_in_network)) {
      ...
      $build['data']['social_links']['message_link'] = [
        '#type' => 'link',
        '#title' => $this->t('Send a message'),
        '#url' => Url::fromRoute('private_message.private_message_create', [], $options),
        '#attributes' => [
    ...
    // If not advanced view mode, return minimal mode flag.
  ...
  /**
   * Get the user access mode when viewing another user.
   *
   * @param \Drupal\Core\Session\AccountInterface $viewed_user
   *   The viewed user.
   *
   * @return string
   *   The user access mode.
   */
  protected function currentUserAccessMode(AccountInterface $viewed_user) {
    $current_user = $this->currentUser();

    // Cache result per request.
    $mode = &drupal_static('giz_user_access_weight_' . $current_user->id() . '_' . $viewed_user->id());
    if ($mode) {
      return $mode;
    }

    // If owner or admin, advanced.
    if ((int) $current_user->id() === 1
      || in_array('administrator', $current_user->getRoles(TRUE), TRUE)
      || $current_user->id() === $viewed_user->id()) {
      $mode = 'advanced';
      return $mode;
    }

    // If connected, advanced.
    $network = $this->connectionsManager->getUserNetwork($current_user->id());
    if (in_array($viewed_user->id(), $network, FALSE)) {
      $mode = 'advanced';
      return $mode;
    }
  ...
  private function getAchievementImagePath(string $image): string {
    $path = $this->themeExtensionList->getPath('aristotle') . "/dist/images/design/$image";

    return file_exists($path) ? $this->fileUrlGenerator->transformRelative(base_path() . $path) : '';
  }
  ...
  protected function buildModuleDetails(
    ...
    $module = $this->entityTypeManager()->getStorage('opigno_module')->load($step['id']);
    ...
    return [
      ...
      [
        ...
        [
          ...
          '#value' => $step['name'] . ' ' . (!empty($completed_on) ? $this->t('completed') : ''),
        ],
      ],
      [
        '#type' => 'html_tag',
      ...
      [
        ...
        !empty($completed_on) ? [
          '#type' => 'html_tag',
          '#tag' => 'p',
          '#value' => $this->t('@name completed on @date', [
            '@name' => $step['name'],
            '@date' => $completed_on,
          ]),
        ] : [],
        !($max_score == 0) ? [
          '#type' => 'html_tag',
          '#tag' => 'p',
          '#value' => $this->t('User got @score of @max_score possible points.', [
            '@score' => $score,
            '@max_score' => $max_score,
          ]),
        ] : [],
        !($max_score == 0) ? [
          '#type' => 'html_tag',
          '#tag' => 'p',
          '#value' => $this->t('Total score @percent%', [
        ...
        [
          ...
          '#value' => $this->t('Activities Overview'),
        ],
  ...
  protected function getTargetAttempt(array $attempts, OpignoModule $module) {
    if ($module->getKeepResultsOption() == 'newest') {
      ...
      $attempts = array_filter($attempts, function ($attempt) use ($module) {
        return $attempt->module->target_id == $module->id();
      });
  ...
  public function buildTrainingDetails(UserInterface $user, GroupInterface $group) {
    ...
    if ($expired) {
      ...
      $query = $this->database
        ...
        ->condition('sa.parent_id', 0);
      $modules = $query->execute()->fetchAll();
      $passed_modules = array_filter($modules, static function ($module) {
