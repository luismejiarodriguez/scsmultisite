...
class H5PUploadWidget extends H5PWidgetBase {
  ...
  public function formElement(FieldItemListInterface $items, $delta, array $element, array &$form, FormStateInterface $form_state) {
    ...
    if (!$element['id']['#value']) {
      $element['file'] = [
        '#name' => "files[{$element['element_path']['#value']}]",
        '#type' => 'file',
        '#title' => t('H5P Upload'),
      ...
      $element['file'] = [
        '#name' => "files[{$element['element_path']['#value']}]",
        '#markup' => t('You already have uploaded a H5P element, delete it by selecting <strong>Clear content</strong> checkbox below and saving the node.'),
      ];
    }
  ...
  public function validate($element, FormStateInterface $form_state) {
    preg_match("/\[([a-z0-9_]+)\]/", $element['#name'], $match);
    $file_field = $match[1];
    ...
    if (version_compare(\Drupal::VERSION, '10.2.0', '>=')) {
      $validators['FileExtension']['extensions'] = 'h5p';
    }
    // @todo Remove this, once we drop the Drupal 10 support:
    else {
      $validators = ['file_validate_extensions' => ['h5p']];
    }
  ...
  protected function massageFormValue(array $value, $delta, $do_new_revision) {
    ...
    $file_is_uploaded = (isset($value['file']) && $value['file'] === 1);
    if (!$file_is_uploaded) {
      return $return_value; // No new file, keep existing value
    }

    // Store the uploaded file
    $storage = H5PDrupal::getInstance('storage', $value['element_path']);
