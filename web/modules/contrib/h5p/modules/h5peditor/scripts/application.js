  ...
  Drupal.behaviors.H5PEditor = {
    attach: function (context, settings) {
      $(once('H5PEditor','.h5p-editor', context)).each(function () {
        ...
        // Grab data values specific for editor instance.
        var $this = $(this);
        ...
        var submit = {
          ...
          handler: function (event) {
            if (h5peditor !== undefined) {
              // Force blur so focused inputs have a chance to save content.
              document.activeElement.blur();

              let params = h5peditor.getParams();

              // TODO - Calculate & set max score
              // $maxscore.val(h5peditor.getMaxScore(params.params));

              // Update input fields to persist values.
              // Set main library.
              ...
              if (params !== undefined && params.params !== undefined) {
                // Validate mandatory main title. Prevent submitting if that's
                // not set. Deliberately doing it after getParams(), so that any
                // other validation problems are also revealed.
                if (event && event.preventDefault && !h5peditor.isMainTitleSet()) {
